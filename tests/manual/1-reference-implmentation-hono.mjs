...
const { routePath, serverPort, cfgSchemaImporter, cfgFilterArrayOfSchemas } =
	config;
...
const arrayOfSchemas = await SchemaImporter.loadFromFolder(cfgSchemaImporter);
const { filteredArrayOfSchemas } = FlowMCP.filterArrayOfSchemas({
	arrayOfSchemas: arrayOfSchemas.map(({ schema }) => {
		return schema;
	}),
	...cfgFilterArrayOfSchemas,
});
const { arrayOfMcpTools } = filteredArrayOfSchemas.reduce((acc, schema) => {
